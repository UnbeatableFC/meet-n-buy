...
export const formatTimestamp = (timeStamp, showTime = false) => {
  if (!timeStamp) {
    return ""; // or any default string for missing timestamps
  }

  // Convert Firestore Timestamp to JS Date
  const date = timeStamp.toDate ? timeStamp.toDate() : new Date(timeStamp.seconds * 1000 + timeStamp.nanoseconds / 1e6);
